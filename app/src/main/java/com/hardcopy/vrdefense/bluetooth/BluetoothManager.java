...
public class BluetoothManager {
     ...
     * Single pattern
     */
    public synchronized static BluetoothManager getInstance(Context c, Handler h) {
        if(mInstance == null) {
            if(c != null)
                mInstance = new BluetoothManager(c, h);
            else
                return null;
        }
        return mInstance;
    ...
    /**
     * Callback handler. Supports only one handler but if you want, modify here.
     * @param h  A Handler to send messages back to the UI Activity
     */
